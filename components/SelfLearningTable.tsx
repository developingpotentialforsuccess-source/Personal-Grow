...
`;
    ...
    import('../services/convex').then(({ saveTopic }) => {
      ...
      if (storedUser) {
        try {
          const u = JSON.parse(storedUser);
          if (u.uid) {
            saveTopic(u.uid, topicToMove, 'selfLearning');
            if (targetParentId) {
              const newParent = findSpecificTopic(updated, targetParentId);
              if (newParent) saveTopic(u.uid, newParent, 'selfLearning');
            }
          }
        } catch(e){}
      }
  ...
  const renderTopic = (topic: DPSSTopic, depth = 0, numberPrefix = '') => {
  ...
  return (
    <div className="flex flex-col md:flex-row h-full w-full p-0 gap-0 overflow-hidden relative">
      ...
      <div 
        ...
        `}
      >
        <div className="flex items-center justify-between mb-1 shrink-0 w-full relative">
          <div className="w-10 shrink-0" />
          <h2 className="text-xl font-black text-slate-800 tracking-tight whitespace-nowrap flex-1 text-center">Self-Learning</h2>
          <button 
            ...
            <div className="space-y-1 min-h-[50px] outline-none rounded-xl transition-all">
               {(() => {
                 if (sidebarFilter === 'smart') {
                   ...
                   return (
                     <>
                      <div className="text-[9px] font-black uppercase tracking-widest pl-1 mb-2 flex items-center justify-between gap-1 text-emerald-500">
                        <span className="flex items-center gap-1">
            ...
            {isTooLargeForCloud(sharingTopic) && (
              <div className="p-3 bg-emerald-500/10 border border-emerald-500/20 text-emerald-600 dark:text-emerald-450 rounded-xl text-xs flex flex-col gap-1">
                ...
                <p className="text-[11px] leading-relaxed text-slate-600 dark:text-slate-300">
                  This folder is <strong>{getTopicSizeString(sharingTopic)}</strong>. Our cloud system handles large payloads efficiently, so you can share it seamlessly!
                </p>
              </div>
            )}
        ...
        <div className="fixed inset-0 bg-slate-950/60 backdrop-blur-md flex items-center justify-center p-4 z-[99999] animate-fade-in font-sans">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 rounded-[24px] max-w-lg w-full shadow-2xl flex flex-col max-h-[85vh]">
            ...
            <div className="flex-1 overflow-y-auto py-3 min-h-0 flex flex-col">
              ...
              <div className="space-y-1.5 max-h-[250px] overflow-y-auto pr-1 flex-1">
                ...
                {(() => {
                  ...
                  const destinations = getMoveDestinations(data.selfLearningTopics || [], idExcludes);
            ...
            <div className="pt-3 border-t border-slate-100 dark:border-slate-800 shrink-0">
              ...
              <button
                onClick={() => {
                  moveTopicToNoteTaking(moveTargetTopic);
                  setMoveTargetTopic(null);
                }}
